...
class NeuralNet:
    def __init__(self, input_size, num_hidden_layers, output_size, width, activation_func, loss_func, learning_rate,
        ...
        if activation_func not in self.activations:
            print(f"error, {activation_func} not a supported activation function")
            return
    ...
    def backward(self, y):
        ...
        for i in reversed(range(len(self.layers))):
            if self.layers[i].layer_type is "input":
                ...
                output_weights_deriv = (last_hidden_layer.output.transpose() @ output_delta)
                # output_weights_deriv = gradient_clip(output_weights_deriv, threshold)
                output_new_weights = output_layer.weights - (self.lr * output_weights_deriv)
                ...
                hidden_activation_deriv = hidden_layer.activation_function_grad_function(hidden_layer.output)
                ...
                new_weights = hidden_layer.weights - (self.lr * hidden_weights_deriv)
...
class Layer:
    def __init__(self, layer_type, activation_function, input_size, output_size):
        ...
        def delta_cross_entropy(x, y):  # taken from https://deepnotes.io/softmax-crossentropy
            """
...
def save_net(neural_net, filename):
    filehandler = open(PATH_NAME+filename+".pkl", 'wb+')
    dill.dump(neural_net, filehandler, fmode="wb+", protocol= dill.HIGHEST_PROTOCOL)
    filehandler.close()


def open_net(filename):
    filehandler = open(PATH_NAME+filename+".pkl", 'rb+')
...
def make_and_train_net(num_hidden_layers=2, width=200, activation_func="ReLU", learning_rate=0.005, dropout=0.5,
    ...
    input_size = 784
    output_size = 10
    train_data = MnistDataset("trainingSet")
    ...
    temp_net.fit(train_data.image_tensors, train_data.image_labels, epochs=int(epochs))
    save_net(temp_net, net_name)
    return net_name


def predict_image(path, neural_name="test"):
    temp = Image.open(fp=path).convert("1")
    temp.load()
    temp = temp.resize((28, 28))
    temp_tensor = np.asarray(temp, dtype="int64")
